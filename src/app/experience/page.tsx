      ...
      <div className="relative w-full mx-auto px-4 py-20">
        ...
        <div className="flex flex-col relative">
          {TIMELINE_DATA.map((item, index) => {
            ...
            return (
              <div key={index} className="relative w-full mb-16 lg:mb-12">
                ...
                <div className={`flex items-center justify-between w-full ${isEven ? 'lg:flex-row' : 'lg:flex-row-reverse'}`}>

                  <div className="w-full lg:w-[45%] z-20 pl-20 lg:pl-0">
                    <Box className="p-6 hover:shadow-xl transition-shadow duration-300">
                    <span className="text-xs font-bold text-primary/80 uppercase tracking-widest block mb-2">
                      {item.date}
                    </span>

                      <h3 className="text-xl font-bold">{item.title}</h3>

                      <div className="flex flex-col sm:flex-row sm:justify-between mb-3 border-b border-zinc-100 dark:border-zinc-800 pb-2 text-sm italic opacity-70">
                        <p>{item.employer}</p>
                        <p>{item.location}</p>
                      </div>
